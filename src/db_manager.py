...
class DatabaseManager:
    ...
    def add_default_commands(self):
        ...
        for command_name, trigger_phrase in default_commands:
            self.cursor.execute("SELECT * FROM commands WHERE command_name = ?", (command_name,))
            result = self.cursor.fetchone()
            if not result:
                self.cursor.execute("INSERT INTO commands (command_name, trigger_phrase) VALUES (?, ?)", (command_name, trigger_phrase))
        self.conn.commit()
    ...
    def update_trigger_phrase(self, command_name, new_trigger_phrase):
        ...
        self.cursor.execute("UPDATE commands SET trigger_phrase = ? WHERE command_name = ?", (new_trigger_phrase, command_name))
        self.conn.commit()
