...
class AssistantGUI:
    ...
    def __init__(self):
        ...
        self.current_category_frame = None
        self.command_entries = {}

        self.root.mainloop()

    def create_main_screen(self):
        """Creates the main UI screen with entry fields and buttons."""
        self.ui_elements["name_label"] = tk.Label(self.root, text="Your Name:")
        self.ui_elements["name_label"].pack(pady=5)

        self.ui_elements["name_entry"] = tk.Entry(self.root, width=30)
        self.ui_elements["name_entry"].pack(pady=5)
    ...
    def start_assistant(self):
        ...
        if not user_name:
            self.ui_elements["status_label"].config(
                text="Please enter a name!", fg="red")
            return
    ...
    def update_ui_after_start(self):
        ...
        self.ui_elements["status_label"].config(
            text="Assistant is running!", fg="green")
        self.ui_elements["name_label"].pack_forget()
        self.ui_elements["name_entry"].pack_forget()
        self.ui_elements["start_button"].pack_forget()
    ...
    def show_category_list(self):
        ...
        for i, (label, category) in enumerate(categories):
            button = tk.Button(
                button_frame, text=label, command=lambda c=category: self.display_commands(
                    c)
            )
    ...
    def display_commands(self, category):
        ...
        for _, command_name, trigger_phrase in commands:
            self.create_command_entry_frame(
                command_name, trigger_phrase).pack(pady=2, padx=10, fill="x")

        tk.Button(self.current_category_frame, text="Save Changes",
                  command=self.save_changes).pack(pady=10)
        tk.Button(self.current_category_frame, text="Back",
